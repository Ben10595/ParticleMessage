...
export function createTextLayout(text: string, options: TextOptions): TextLayout {
  ...
  if (!layout) {
    ...
    const spacing = options.spacing ?? (options.dotMatrix ? Math.max(1.05, Math.min(1.5, fontSize / 13)) : Math.max(1.05, Math.min(3.25, fontSize / 28)));
    const resolution = 2;
    ...
    for (const glyph of glyphs) {
      ...
      for (let py = top + spacing / 2, row = 0; py < Math.min(height, glyph.y + fontSize * .65); py += spacing, row++) {
        ...
        for (let px = start, col = 0; px < Math.min(width, glyph.x + glyph.width); px += spacing, col++) {
          ...
          if (alpha >= .23) targets.push({ x: px, y: py, size: spacing * (options.dotMatrix ? .44 : fontSize < 40 ? .46 : .4), opacity: Math.min(1, .8 + alpha * .2), key: `glyph-${glyph.index}-${row}-${col}`, glyph: glyph.index, delay: 0 });
        }
      }
    }
