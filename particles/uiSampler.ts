...
export function sampleUI(root: HTMLElement, viewportHeight: number): UISample {
  ...
  root.querySelectorAll<HTMLElement>('[data-particle]').forEach(element => {
    ...
    const text = element.dataset.text ?? (kind === 'control' && element instanceof HTMLSelectElement ? element.selectedOptions[0]?.textContent : element.textContent) ?? '';
    const fontSize = parseFloat(style.fontSize);
    const layout = createTextLayout(text, { x: rect.x + left, y: rect.y + top, width: rect.width - left - right, height: rect.height - top - bottom, fontSize, nowrap: kind !== 'hero' && rect.height < fontSize * 2.6, weight: Math.max(500, Number(style.fontWeight) || 500), spacing: fontSize < 18 ? .85 : fontSize < 24 ? 1.05 : undefined, align: style.textAlign === 'center' ? 'center' : 'left', fit: true });
    layouts.push(layout);
    targets.push(...layout.targets.map(p => ({ ...p, key: `${key}-${p.key}`, size: fontSize < 18 ? .45 : fontSize < 24 ? .5 : p.size, opacity: disabled ? .28 : kind === 'hero' || kind === 'button' || active ? 1 : .92, delay: Number(element.dataset.particleDelay ?? 0) + (p.glyph ?? 0) * (kind === 'hero' ? 15 : 3) })));
