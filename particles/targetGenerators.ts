...
export function createRectangleTargets(x: number, y: number, width: number, height: number, spacing = 7, speed = 14): Target[] {
  ...
  const left = createLineTargets(x, y + height, x, y, spacing, speed).slice(0, -1);
  const all = [...top, ...right, ...bottom, ...left];
  const perimeter = 2 * (width + height);

  return all.map((target, index) => {
    let offset = 0;
    if (index < top.length) {
      offset = target.x - x;
    } else if (index < top.length + right.length) {
      offset = width + (target.y - y);
    } else if (index < top.length + right.length + bottom.length) {
      offset = width + height + (x + width - target.x);
    } else {
      offset = width * 2 + height + (y + height - target.y);
    }
    return {
      ...target,
      theme: 'cool',
...
export function createIconTargets(icon: 'plus' | 'close' | 'arrow' | 'heart', x: number, y: number, size = 18): Target[] {
  ...
  return Array.from({ length: 36 }, (_, i) => { const t = i / 36 * Math.PI * 2; return { x: x + half * Math.sin(t) ** 3, y: y - size / 32 * (13 * Math.cos(t) - 5 * Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t)), size: .8, theme: 'cool' }; });
}
