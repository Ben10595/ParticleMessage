...
export async function loadMessage(slug: string, signal?: AbortSignal): Promise<MessageContent | null> {
  if (!SLUG_PATTERN.test(slug)) return null;
  const { data, error } = await getSupabase().from('messages').select('content').eq('slug', slug).abortSignal(signal ? AbortSignal.any([signal, AbortSignal.timeout(15000)]) : AbortSignal.timeout(15000)).single();
  if (error?.code === 'PGRST116') return null;
  if (error) throw new Error('Die Nachricht konnte nicht geladen werden. Bitte versuche es erneut.');
  return validateMessage(data?.content);
}
